...
SIFT is a local-first semantic search engine for personal knowledge management. It provides hybrid search (BM25 + vector embeddings) with reranking, feedback-driven scoring, and usage tracking. SIFT is the retrieval engine that powers [Mem](../mem/) - a personal memory system for AI agents.
...
[embedding]
model = "voyage-4-lite"
dimensions = 1024              # Options: 256, 512, 1024, 2048
batch_size = 128               # Max texts per Voyage API call (API limit: 1000)
...
enabled = true
top_n = 50                     # Rerank top N from initial retrieval

[chunking]
rows_per_chunk = 45
overlap_rows = 5               # Context: 5 rows before + 5 rows after
min_chunk_chars = 25           # Skip chunks below this threshold
...
Send top 50 (configurable) chunks to Voyage `rerank-2.5-lite` with original query. Returns relevance_score for each.
...
2. Group into chunks of N rows (default: 45, configurable via `chunking.rows_per_chunk`)
3. Store chunk as (file_id, chunk_order, start_line, end_line)
4. When embedding: extract lines with overlap (±5 rows)
5. Skip chunks with < min_chunk_chars (default: 25)
6. Log char_count per chunk for monitoring

### Overlap Semantics

```
Chunk 0: lines 1-45   → embed with context lines 1-50
Chunk 1: lines 46-90  → embed with context lines 41-95
Chunk 2: lines 91-135 → embed with context lines 86-140
```
...
- **Model**: `voyage-4-lite` (1024 dims default)
- **Auth**: `Authorization: Bearer $VOYAGE_API_KEY`
- **Batch**: Up to 128 texts per request (API supports 1000, but 128 is safer)
...
│   │   └── hybrid.go          # RRF fusion
│   ├── chunk/
│   │   ├── chunker.go         # Chunker interface + row-based impl
│   │   └── content.go         # Content extraction with overlap
│   ├── search/
